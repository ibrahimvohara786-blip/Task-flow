...
const updateThemeElements = (theme) => {
    ...
    if (theme === 'dark') {
        ...
        themeBtn.innerHTML = '<i class="fa-solid fa-desktop"></i>';
    } else {
...
const applyTheme = (theme) => {
    ...
    document.body.classList.toggle('dark', theme === 'dark' || (theme === 'system' && prefersDarkScheme.matches));
...
prefersDarkScheme.addEventListener('change', updateSystemTheme);
...
const dwProgressBar = document.getElementById('dwProgressBar');
const dwCustomMinutes = document.getElementById('dwCustomMinutes');
const dwSetCustom = document.getElementById('dwSetCustom');
...
const dwPresetButtons = document.querySelectorAll('.dw-preset');
const dwGoalLabel = document.getElementById('dwGoalLabel');
const dwModeLabel = document.getElementById('dwModeLabel');

let dwInterval = null;
let dwDuration = 25 * 60;
let dwSeconds = dwDuration;
let dwRunning = false;

const renderTimer = () => {
    if (dwTimerEl) dwTimerEl.textContent = formatTime(dwSeconds);
    const duration = dwDuration || 1500;
    if (dwProgressBar) dwProgressBar.style.width = `${Math.min(100, Math.round(((duration - dwSeconds) / duration) * 100))}%`;
};

const setPreset = (minutes) => {
    dwDuration = minutes * 60;
    dwSeconds = dwDuration;
    dwPresetButtons.forEach((button) => button.classList.toggle('active', Number(button.dataset.minutes) === minutes));
    if (dwModeLabel) dwModeLabel.textContent = 'Deep Work';
    renderTimer();
};

const setCustomDuration = (minutes) => {
    if (!minutes || minutes < 1 || minutes > 240) {
        showToast('Enter a custom time between 1 and 240 minutes.');
        return;
    }
    dwDuration = minutes * 60;
    dwSeconds = dwDuration;
    dwPresetButtons.forEach((button) => button.classList.remove('active'));
    if (dwModeLabel) dwModeLabel.textContent = 'Custom Focus';
    renderTimer();
};

const openDeepWork = () => {
    if (deepWorkOverlay) deepWorkOverlay.style.display = 'flex';
    const topTask = tasks.find((task) => !task.completed) || { title: 'Stay focused' };
    if (dwGoalLabel) dwGoalLabel.textContent = `Focus on: ${topTask.title}`;
    renderTimer();
};

const closeDeepWork = () => {
    if (deepWorkOverlay) deepWorkOverlay.style.display = 'none';
...
const startDW = () => {
    ...
    dwInterval = setInterval(() => {
        if (dwSeconds <= 0) {
            clearInterval(dwInterval);
            dwRunning = false;
            showToast('Deep work session completed!');
            return;
        }
        dwSeconds -= 1;
        renderTimer();
...
const resetDW = () => {
    pauseDW();
    const activePreset = Number(document.querySelector('.dw-preset.active')?.dataset.minutes || 25);
    dwSeconds = activePreset * 60;
    renderTimer();
};
...
dwPresetButtons.forEach((button) => {
    button.addEventListener('click', () => {
        setPreset(Number(button.dataset.minutes));
    });
});

dwSetCustom && dwSetCustom.addEventListener('click', () => {
    const minutes = Number(dwCustomMinutes.value);
